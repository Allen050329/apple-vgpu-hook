...
//! fragmentation artifact. See
//! [`MapRefusal::Scattered`](crate::runtime::guest_ram_map::MapRefusal::Scattered).
//!
//! Each run is bindable on its own ([`crate::runtime::guest_ram_map`]), but a
//! run is a range of *bytes* and `vkCmdCopyImageToBuffer` names *rectangles*.
//! A 16 KiB run against a 7680-byte row pitch covers two-and-a-bit rows
//! starting part-way along one, which is not a rectangle. Converting between
//! the two is the whole content of this module.
//!
//! It sits outside the backend tree, and outside every `cfg`, on purpose. It
//! names nothing from `ash` and nothing from `metal`: a region here is four
//! `u32`s and an offset, and both backends can build their own descriptor from
//! one. That is the [`crate::backend::hash`] precedent — pure logic in the
...
/// Bytes per texel. This rail is BGRA8 only — `mapping_write` refuses any other
/// format by name before reaching here, so widening this is a format decision
/// made there and not a constant to generalise on spec.
