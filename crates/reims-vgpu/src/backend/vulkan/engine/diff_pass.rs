...
/// because dropping the whole map would re-seed every live destination at once
/// and cost a full landing each.
#[derive(Default)]
...
/// one more destination evicts one, not the map. Dropping everything costs a
/// full landing for every live destination at once, which no rail can afford.
///
...
/// still sends, taken on the product path and over the frames actually landed —
/// so any boot can be read for it, and nothing has to run a second submission
/// to find out.
